...
from .models import HistoryCandidates, HistoryItem, NewsItem, WeatherReport, MultiCityWeather
...
CATEGORY_HEADERS = {
    ...
    "fotbal_international": "FOTBAL INTERNAȚIONAL (ANGLIA / ITALIA / GERMANIA / SPANIA + CUPE)",
}
...
CATEGORY_ORDER = [
    ...
    "fotbal_romania",
    "fotbal_international",
]


def _format_weather_block(weather: Optional[MultiCityWeather]) -> str:
    if weather is None or not weather.reports:
        return "METEO INDISPONIBIL: menționează scurt că datele meteo nu au putut fi obținute astăzi."
    lines = []
    for report in weather.reports:
        lines.append(
            f"METEO {report.city.upper()} ({report.city})\n"
            f"- Temperatură curentă: {report.temp_current_c:.1f}°C\n"
            f"- Minimă ziua: {report.temp_min_c:.1f}°C\n"
            f"- Maximă ziua: {report.temp_max_c:.1f}°C\n"
            f"- Cer: {report.description}\n"
            f"- Vânt: {report.wind_kmh:.0f} km/h\n"
            f"- Precipitații: {report.precipitation_mm:.1f} mm"
        )
    return "\n\n".join(lines)
...
def build_user_prompt(
    *,
    items: List[NewsItem],
    weather: Optional[MultiCityWeather],
...
SECTIONS: list[Section] = [
    Section(
        ...
        min_words=150,
        guidance=(
            "Citește prognoza pentru cele trei orașe: Moșnița Veche, Timișoara și Reșița. "
            "Menționează pentru fiecare oraș temperatura curentă, minima și maxima ziua, "
            "starea cerului, vântul și precipitațiile. Spune-o ca un prezentator — nu înșira "
            "doar cifre, adaugă o scurtă recomandare practică (umbrelă, haine groase, etc.) "
            "dacă e cazul. Scrie TOATE valorile numerice în cuvinte (ex: opt virgulă unu grade)."
        ),
    ),
    Section(
        key="stiri_locale",
        intro_phrase="Trecem la știrile locale din Moșnița Veche, Timișoara și Caraș-Severin.",
        target_words=400,
        min_words=320,
        guidance=(
    ...
    Section(
        ...
        guidance=(
            ...
            "Liga a II-a. Pentru meciuri, dă scorul, marcatorii (dacă "
    ...
    Section(
        key="fotbal_international",
        intro_phrase="În fotbalul european, rezumăm etapa.",
        target_words=450,
        min_words=360,
        guidance=(
            "Prezintă rezultatele și știrile din Premier League (Anglia), La Liga (Spania), "
            "Bundesliga (Germania), Serie A (Italia), Champions League, Europa League, "
            "Conference League, Cupa Mondială și Cupa Europeană. Grupează pe campionate. "
            "Pentru fiecare meci, dă scorul și o frază de context (cine a marcat, cum e în clasament). "
...
def build_intro(bulletin_date: datetime) -> str:
    ...
    return (
        f"Bună dimineața, Cristi. Astăzi este {format_date_ro(bulletin_date)}. "
        "Iată buletinul de știri."
    )


OUTRO = "Acesta a fost buletinul de astăzi. O zi bună, Cristi!"
